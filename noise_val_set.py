...
def save_img(image, subfolder, imgname, use_JPEG=False):
	...
	if use_JPEG:
		imsave('/data/ILSVRC2012/geirhos_uniform_noise/val_in_folders/' + '/'+subfolder+'/' + imgname, img_as_ubyte(image)) 
	else:
		toimage(image,
				cmin=0.0, cmax=1.0).save('/data/ILSVRC2012/geirhos_uniform_noise/val_in_folders/' + '/'+subfolder+'/' +imgname+".png")
...
if __name__ == "__main__":
	...
	path = '/data/ILSVRC2012/val_in_folders/'
	print('searching for images in ' + path)
	for subdir , _ , dirs in os.walk(path):
		#print('subdirectory: '+subdir)
		filenames = os.listdir(subdir)
		subfolder=os.path.basename(os.path.normpath(subdir))
		#print('subfolder: '+subfolder)
		if not os.path.exists('/data/ILSVRC2012/geirhos_uniform_noise/val_in_folders/'+subfolder):
			os.makedirs('/data/ILSVRC2012/geirhos_uniform_noise/val_in_folders/'+subfolder)
		for item in filenames:
			print('file: '+subdir+'/'+item)
			if os.path.isfile(subdir+'/'+item):
				print('found!')
				
				im = imload_rgb(subdir+'/'+item)
				f, e = os.path.splitext(subdir+'/'+item)

				noise_width = 0.1
				contrast_level_2 = 0.3
				rng = np.random.RandomState(seed=42)

				img_noisy = uniform_noise(image=im, width=noise_width,
								contrast_level=contrast_level_2,
								rng=rng)
				save_img(img_noisy, subfolder, item, use_JPEG)
